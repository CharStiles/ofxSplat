...
void main () {
  ...
  float pos2dy = pos2d.y + cos((sin(time + (pos2d.x * 80. * length(pos2d)) ) ))*0.1;

   // pos2d.y = mix( pos2dy + tan(time + pos2d.y*.9),pos2d.y, clamp(0,1, sin(time/2. + (pos2d.y * 2. * length(pos2d)) ) + 1.) );


   // pos2d.x += tan(cos((pos2d.z + time + length(pos2d.xyz*8.)) * 0.001)) * 100.1;
